	...
	writeRows: config => {
		return new Promise((resolve, reject) => {
			authorize({client_secret: process.env.CLIENT_SECRET,
				client_id: process.env.CLIENT_ID,
				redirect_uris: ['urn:ietf:wg:oauth:2.0:oob', 'http://localhost']
			}, writeToList, resolve, reject, config);
		});
	}
}
...
function authorize(credentials, callback, resolve, reject, config) {
	...
	// in case of need of token refresh
	// getNewToken(oAuth2Client, resolve, reject, config)
 ...
 * @param {object} config
 */
function writeToList(auth, resolve, reject, config) {
	const sheets = google.sheets({version: 'v4', auth});
	const {id, range, reset, value} = config;
	let payload;

	if (value) {
		payload = value;
	} else {
		payload = reset ? Array(reset).fill([0]) : [[1]];
	}
