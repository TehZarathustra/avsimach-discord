...
const initiateRedemption = require('./redemption');
...
function updateMig(message, takeOff) {
	...
	setTimeout(() => {
		if (shouldUseDeps && !areDepsMet(matchedItem.deps)) {
			const msg = `${secondLabel || gluedItem} ${convertedAction.fail}`;
		...
		if (isOnAction && matchedItem.onSuccess) {
			...
			const msg = `${secondLabel || gluedItem} ${matchedItem.status === switchAction
				? ('уже ' + actionSuccess)
				: actionSuccess}`;
	...
	takeOff(state, message)
		...
		.then((isSuccess) => {
			...
			if (isSuccess) {
				...
				missionSuccess && updateProfile({item: 'success'});
			}
...
const mig21Entry = {
	mig21: {
		...
		reply: message => {
			askForMeme(message, 'mig21', updateMig);
		}
	},
	mig21takeoff: {
		pattern: /начальник, взлет/i,
		reply: message => {
			askForMeme(message, 'mig21', updateMig.bind(null, message, true));
		}
	},
	checkMig21: {
