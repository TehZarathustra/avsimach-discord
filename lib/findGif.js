...
const SHOW_REGEX = /(покажи\s|show\s)(.*)$/gi;

function findGif(message) {
	message.content.match(SHOW_REGEX);
	const query = encodeURI(RegExp.$2);
	...
	getImgurGif(null, query)
		...
		.catch(error => error ? message.reply(error) : null);
}
...
module.exports = {
	findGifEntry: {
		findGif: {
			pattern: /начальник, покажи|начальник покажи|boss, show|boss show/i,
			reply: findGifEntry
		}
	}
};
