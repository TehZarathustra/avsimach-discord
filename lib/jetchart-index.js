...
function _compareThrust(first, second) {
	const isBetter = first.thrust > second.thrust;
...
function _compareLoading(first, second) {
	...
	return {
		message: `${first.name} ${isLess ? '**маневреннее**, чем' : '**не такой маневренный**, как'} ${second.name}\n`,
		score: isLess ? 1 : 0
	};
}
...
function _compareSpeed(first, second) {
	...
	return {
		message: `\n${first.name} ${isFaster ? '**быстрее**' : '**медленнее**'}, чем ${second.name}\n`,
		score: isFaster ? 1 : 0
	};
}
...
function _getPlaneByName(planes, name) {
	return planes.filter(plane => plane.name === name || plane.truncateName === name);
}
...
function _transformPlanes(planes) {
	...
	return planes.map(plane => {
		...
		if (plane.name
			...
			&& plane.thrust
			&& plane.wingLoading) {
			plane.fuel = Number(plane.fuel.replace(replaceRegex, ''));
			plane.weight = Number(plane.weight.replace(replaceRegex, ''));
			plane.speed = Number(plane.speed.replace(replaceRegex, ''));
			plane.ceiling = Number(plane.ceiling.replace(replaceRegex, ''));
			plane.thrust = Number(plane.thrust.replace(replaceRegex, ''));
			plane.wingLoading = Number(plane.wingLoading.replace(replaceRegex, ''));
