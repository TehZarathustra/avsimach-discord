...
const {
	...
	compareWeight,
	compareSpeed
} = require('./jetchart-comparison');

const COMPARE_REGEX = /([M|F|S|A]\S*)\sи\s([M|F|S|A]\S*)/gi;

const SPREADSHEET_CONFIG = {
	id: '1SHq0E0nAIcCLE1EU1hz7Y9yEqwNAFE-tPhTc4qfJ6M4',
	range: 'Sheet1!A2:W'
};
...
function compare(firstPlane, secondPlane, message) {
	console.log('memeFn args >>>', arguments);
...
const compareEntry = {
	compare: {
		pattern: /начальник, сравни|начальник сравни/i,
		reply: (message) => {
			message.content.match(COMPARE_REGEX);

			const firstPlane = RegExp.$1;
			const secondPlane = RegExp.$2;
			const args = [firstPlane, secondPlane, message];

			askForMeme(message, 'default', compare.bind(null, ...args));
		}
	}
};

module.exports = {compareEntry, comparePlanes};
